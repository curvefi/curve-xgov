...
def get_destination_network(destination_chain_id: int | str):
    # bridge.networkID() on destination chain
    if int(destination_chain_id) == 1101:  # Polygon
        return 1
    if int(destination_chain_id) == 196:  # xlayer
        return 3
    raise NotImplementedError("Unknown destination chain id")


def get_bridge_url(provider: ProviderAPI):
    rpc = provider.http_uri  # try
    if provider.chain_id == 1101:  # Polygon
        rpc = "https://proof-generator.polygon.technology/api/zkevm/mainnet"
    if provider.chain_id == 196:  # xlayer
        rpc = "https://rpc.xlayer.tech/priapi/v1/ob/bridge"
    return rpc
